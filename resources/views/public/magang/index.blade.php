...
<head>
    <meta charset="UTF-8">
    <title>Magang - AntiNganggur</title>
...
<body class="min-h-screen bg-white text-gray-800 flex flex-col">
    ...
    <nav class="bg-gray-900 text-white px-6 py-4 shadow">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div class="space-x-6 text-sm md:text-base flex justify-center">
                <a href="{{ route('pencaker.dashboard') }}" class="hover:text-purple-300">Dashboard</a>
                <a href="{{ route('public.lowongan') }}" class="hover:text-purple-300">Lowongan</a>
                <a href="{{ route('public.magang') }}" class="text-purple-400 font-semibold">Magang</a>
    ...
    <div class="min-h-screen bg-gradient-to-br from-purple-100 via-white to-blue-100 text-gray-900 p-10">
        <h1 class="text-3xl font-bold text-center mb-10">💼 Magang Tersedia</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($magangs as $m)
                <div class="bg-white rounded-lg shadow-lg p-6 border-t-4 border-green-500 hover:shadow-xl transition">
                    <h3 class="text-lg font-semibold">{{ $m->keahlian }}</h3>
                    <p class="text-sm text-gray-600 mt-1">Min Pendidikan: {{ $m->min_pendidikan }}</p>
                    <p class="text-sm text-gray-600 mt-1">Gaji: Rp{{ number_format($m->gaji, 0, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 mt-2">📧 {{ $m->email }} | 📱 {{ $m->no_telp }}</p>
